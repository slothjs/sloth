...
Sloth.prototype.buildIndex = function (callback) {
  ...
  fs.readFile(ignoreFile, 'utf-8', function (err, data) {
    ...
    return filesystem.crawl(self.directory, ignore, function (vinyl) {
      /**
       * Create the file (in build/ or something)
       * Pipe to any transforms
       * Pipe the result into the new file
       * Done!
       */
    }).then(function (data) {
      self.index = data.toObject();
      console.log(self.prettyPrint());
    }).fail(function (err) {
      console.log(err);
    });
  });
